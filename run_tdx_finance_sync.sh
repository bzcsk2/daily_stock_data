...
TIMEOUT_SECONDS="${TDX_FINANCE_TIMEOUT:-5400}"
WORKERS="${TDX_FINANCE_WORKERS:-4}"
CHUNK_SIZE="${TDX_FINANCE_CHUNK_SIZE:-250}"
...
if timeout "${TIMEOUT_SECONDS}" "${PYTHON_BIN}" sync_tdx_finance.py --workers "${WORKERS}" --chunk-size "${CHUNK_SIZE}" >> "${LOG_FILE}" 2>&1; then
  ...
  rc=$?
  echo "$(date '+%F %T %z') [WARNING] pytdx finance sync exit_code=${rc}" | tee -a "${LOG_FILE}"
  exit "${rc}"
fi
